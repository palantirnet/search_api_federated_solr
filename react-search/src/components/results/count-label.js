...
function searchResultsStat(currentPage, numFound, rows, pageAmt) {
  if (numFound > rows) { // Many pages
    ...
    return (
      <p id="stat" tabIndex="-1" className="search-results-stat">Your search yielded no results.</p>
    )
  }
}
...
class FederatedCountLabel extends React.Component {
    ...
    const pageAmt = Math.ceil(numFound / rows);
		...
		return (
        <React.Fragment>
          {searchResultsStat(currentPage, numFound, rows, pageAmt)}
        </React.Fragment>
		);
	}
}
